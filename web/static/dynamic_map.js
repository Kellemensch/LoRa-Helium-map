...
// Chargement des données pour une date spécifique
function loadDateData(date) {
    ...
    // Afficher "Loading..." dans le panneau de date
    document.getElementById('currentDate').textContent = `${date} (Loading...)`;
    
    // Récupérer les données pour cette date
    fetch(`/api/gateways?date=${date}`)
        .then(response => response.json())
        .then(data => {
            ...
            data.forEach(gw => {
                ...
                let infoHtml = `
                    <b>Gateway Name:</b> ${gw.gateway_name}<br>
                    <b>Gateway ID:</b> ${gw.gatewayId}<br>
                ...
                L.circleMarker(gwLocation, {
                    ...
                    fillColor: color,
                    fillOpacity: 0.8,
                    title: `${gw.gateway_name} - ${date}`
                }).addTo(currentLayer);
                
                // Petit cercle cliquable
                L.circleMarker(gwLocation, {
                    radius: 8,
                    color: color,
                    fillColor: color,
        ...
        .catch(error => {
            ...
            document.getElementById('currentDate').textContent = `${date} (Error loading data)`;
        });
}

// Mise à jour des contrôles de date
